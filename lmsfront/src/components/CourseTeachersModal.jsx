...
const CourseTeachersModal = ({ course, onClose }) => {
  const { fetchListUser, addTeacherToCourse, removeTeacherFromCourse } = useAdmin();

  const [allUsers, setAllUsers]   = useState([]);
  const [search, setSearch]       = useState('');
  const [loadingId, setLoadingId] = useState(null);

  const enrolledIds = new Set((course.teachers || []).map(t => t.user_id));
  ...
  const handleAdd = async (user_id) => {
    ...
    try {
      await addTeacherToCourse(course.id, [user_id]);
      const u = allUsers.find(u => u.user_id === user_id);
      if (u) course.teachers = [...(course.teachers || []), u];
    } catch (e) {
      console.error(e);
    } finally {
      setLoadingId(null);
  ...
  const handleRemove = async (user_id) => {
    setLoadingId(user_id);
    try {
      await removeTeacherFromCourse(course.id, [user_id]);
      course.teachers = (course.teachers || []).filter(t => t.user_id !== user_id);
    } catch (e) {
      console.error(e);
    } finally {
      setLoadingId(null);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-card rounded-2xl border border-border w-full max-w-lg max-h-[80vh] flex flex-col shadow-xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-foreground">Управление преподавателями</h2>
        ...
        <div className="px-6 py-2 shrink-0">
          <p className="text-xs text-muted-foreground">
            Преподавателей на курсе: <span className="font-medium text-foreground">{course.teachers?.length || 0}</span>
          </p>
        </div>
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-xl border transition ${
                  isEnrolled ? 'border-purple-500/30 bg-purple-500/5' : 'border-border'
                }`}
              >
                <div className="w-8 h-8 rounded-full bg-purple-500/10 flex items-center justify-center text-xs font-bold text-purple-500 shrink-0">
                  {u.last_name?.[0] || '?'}
                </div>
                ...
                {isEnrolled ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium text-purple-500 border border-purple-500/20 hover:bg-purple-500/10 transition disabled:opacity-50"
                  >
                    {isLoading
                      ? <Loader2 size={12} className="animate-spin" />
                      : <UserPlus size={12} />
...
export default CourseTeachersModal;
